...
## 三·五、户籍录入准入原则（2026-08-18 用户确认）

> 🔴 户籍不是"提到就录入"，是"有关系才录入"。被描述/引用但无明确关系的人（含名人、幻想对象、无关人员）**不进入户籍**，防止无关人员堆积导致户籍管理复杂化。

| 准入类型 | 触发方式 | 判定 | 依据 |
|---------|---------|------|------|
| ① 明确关系声明 | 用户说「XX是我妈妈/朋友/同事…」即 `我{称谓}叫{名字}` | ✅ 确定录入 | 规则路径（`extractNamedKinshipMentions` 正则，确定性） |
| ② 明确录入指令 | 用户说「玉瑶，把XX加进来 / 认识一下XX」 | ✅ 确定录入 | LLM/规则提取 + FG 写入 |
| ③ 仅被描述/引用 | 用户只提到名字，未声明关系（如"刘亦菲好美"、"XX拍戏很火"） | ❌ **不录入** | 实体识别层（M1 规则 / LLM 提取）不产出 person 实体，写入层无从建档 |

**执行机制（现状，不改代码）**：
- 录入的判定在**实体识别层**（M1 规则提取 + LLM 提取），不在写入层。
- 写入层（`FGProfileWriteGateway.canWriteEntity`）只做会晤写隔离，不改变"谁该录入"的判定。
- 刘亦菲案例：LLM 提取器将名人/幻想对象视为"非用户身边人"而排除 → `entity_genes` 无刘亦菲 → FG 无从建档。**此行为被确认为设计正确，保持。**

**边界说明**：会晤模式中谈论的第三人（非会晤实体）同样遵循此原则——无关系声明不录入。若用户明确要求（如"刘亦菲是我认识的人，加上她"），走准入类型 ①/② 录入。
