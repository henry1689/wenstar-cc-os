...
function isAnswerSentence(s: string): boolean {
    ...
    if (_paren) {
        ...
        if (/^(?:因为|由于|关于|反映|准备|注意|我的|让我|我先|开场|身份|自称|这是|嗯|反应|比如|例如|像|好像|属于|但先|只是|不过|整理|录入|快到午饭|拟定|规划)/.test(_inner))
            return false;
        // ④c V14修复: 括号段含引号+评估语气（"（"怎么还没开始"/"快点开始"），衔接了当前场景，而且保持了克制。）"）
        //   ——模型在最终稿前的评估句（引用用户/自己话+衔接/保持/克制等评估动词）→ 非答案
        if (/[“"'"]/.test(_inner) && /(?:衔接|保持|克制|这句|这样|这个|语气|回应|回答|表达|放在|自然|得体)/.test(_inner))
            return false;
