...
const THINKING_KEYWORDS = /让[我你]想|让我回|让我分析|让我来写|我来写|心里|想到|脑中|好好回|在意|吃醋|心酸|我们被问|当前场景|当前时间|我需要|考虑到|根据规则|从历史|在角色扮演|但根据|用户最后|用户可能|用户当前|我的回复|这个角色|最安全|但注意|可能这是|我决定|最简单的做法/;
/**
...
const REFLECTIVE_SIGNAL_RE = /【[^】]*(?:⚠️|🔴|🟡|禁止|必须|规则|指令|优先|模式|要求|注意|事实|记忆)[^】]*】|系统提醒|系统指令|系统告诉|系统要求|事实回忆|事实优先|记忆优先|不能编造|不要编造|不要添加|绝对禁止|提醒过|根据规则|当前问题是事实|让我分析一下|让我来写|分析一下场景|现在说话的是|让我再完善|让我再重写|让我定稿|让我稍微收紧|再检查一遍|嗯，这很好|让我确认一下|让我收紧|让我微调/;
...
const REFLECTIVE_GO_RE = /开始[。！？]|开始吧|让我来写[^。]{0,10}(?:回应|回答|一段|回复)[：:]|让我来写[^。]{0,10}[：:]|这样回应吧|让我再完善[^。]{0,20}[：:.]|让我再重写[^。]{0,20}[：:.]|让我定稿|让我稍微收紧[^。]{0,10}[：:.]|让我微调|让我收紧|嗯，这很好[^。]{0,30}让我定稿|再检查一遍/g;
