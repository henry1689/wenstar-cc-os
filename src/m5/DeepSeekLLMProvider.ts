...
const REFLECTIVE_SIGNAL_RE = /【[^】]*(?:⚠️|🔴|🟡|禁止|必须|规则|指令|优先|模式|要求|注意|事实|记忆)[^】]*】|系统提醒|系统指令|系统告诉|系统要求|事实回忆|事实优先|记忆优先|不能编造|不要编造|不要添加|绝对禁止|提醒过|根据规则|当前问题是事实/;
 ...
 */
// V12根治: 身体动作词（动作描写答案 vs 思维链分析/场景括号的泛化区分）
const ACTION_VERBS = /愣|笑|顿|停|缩|颤|叹|咽|抬|低|咬|吸|摇|点|垂|眯|皱|抿|僵|怔|埋|补|抖|贴|枕|挪|坐|站|走|看|听|屏|沉|放低|放缓|清了清|深呼吸|拍了拍|抚上|贴了贴|顿了顿|停了停|放慢|压得/;
...
function isAnswerSentence(s: string): boolean {
    ...
    if (/^(?:鸿艺先生|鸿艺|梓铭|玉瑶|诗雨|徐诗雨)[，,][\s\S]{0,60}(?:你|您|我)/.test(s))
        return true;
    // V12修复: 称呼+逗号+对话——句子以"鸿艺先生，"等开头且含"你/您/我" → 强答案信号（排除模拟对话）
    if (/^(?:鸿艺先生|鸿艺|梓铭|玉瑶|诗雨|徐诗雨)[，,][\s\S]{0,60}(?:你|您|我)/.test(s) && !/(?:我可以说|然后说|比如|像|仿佛|装作|假装|应该说)/.test(s.slice(0, 40)))
        return true;
