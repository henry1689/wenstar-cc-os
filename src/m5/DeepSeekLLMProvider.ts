...
const REFLECTIVE_SIGNAL_RE = /【[^】]*(?:⚠️|🔴|🟡|禁止|必须|规则|指令|优先|模式|要求|注意|事实|记忆)[^】]*】|系统提醒|系统指令|系统告诉|系统要求|事实回忆|事实优先|记忆优先|不能编造|不要编造|不要添加|绝对禁止|提醒过|根据规则|当前问题是事实|让我分析一下|让我来写|我来写[^。]{0,10}(?:回应|一段)|分析一下场景|现在说话的是/;
...
const REFLECTIVE_GO_RE = /开始[。！？]|开始吧|让我来写[^。]{0,10}(?:回应|回答|一段|回复)[：:]|我来写[^。]{0,10}(?:回应|回答|一段|回复)[：:]|让我来写[^。]{0,10}[：:]|这样回应吧/g;
