...
export class CoreMemoryManager {
  ...
  async refreshFromProfile(): Promise<void> {
    try {
      ...
      if (synth.topConcerns.length > 0) {
        const concerns = synth.topConcerns.slice(0, 3).map((c: any) => c.topic.replace(/^(偏好|习惯|信息):\s*/, '')).join('、');
  ...
  /** 🎭 角色扮演模式下覆写 persona 块（阻止"你的名字是玉瑶"泄漏） */
  setRoleplayOverride(charName: string, relationCtx: string): void {
    this.blocks.set('persona', {
      label: 'persona',
      value: `【身份锁定·最高优先级】你现在是「${charName}」，不是玉瑶，不是任何其他人。忘记你原来叫玉瑶的身份。${relationCtx}`,
      sizeLimit: 300,
      priority: 100,
      updatedAt: new Date().toISOString(),
    });
