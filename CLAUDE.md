...
### 6. 🔴 代码变更后 → 五道防线核验（不可跳过）
任何 `.ts`/`.js` 文件被修改后，提交前或宣布"完成"前，必须走完五道防线：
1. **编译** — `npx tsc --noEmit`，错误数不能比改前多
2. **功能** — 用真实输入驱动改过的功能，确认输出正确
3. **牵连** — 改了 A 就检查所有依赖 A 的模块（`grep -rn "from.*<A>" src/`），逐一确认未被破坏
4. **服务** — 如果是运行时改动，必须启动服务器验证 `health` 正常
5. **监控/前端** — 核查看板、相关页面正常渲染

五道全过 → 才能报完成。有一道不过 → 回退→诊断→修→重验。

详见 `.claude/skills/verify.md`。
